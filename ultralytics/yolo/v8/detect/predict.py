...
class DetectionPredictor(BasePredictor):
    ...
    def write_results(self, idx, preds, batch):
        ...
        # Combine the counts for each class
        for key, value in object_counter.items():
            total_vehicle_counts[key] = value + object_counter1.get(key, 0)

        # Get the directory from save_path to construct the CSV file path
        video_dir = os.path.dirname(save_path)
        csv_file_path = os.path.join(video_dir, "vehicle_counts.csv")

        with open(csv_file_path, mode='w', newline='') as csvfile:
            fieldnames = ['Vehicle Type', 'Entering Count', 'Leaving Count', 'Total Count']
            writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
            writer.writeheader()

            for vehicle_type, count in total_vehicle_counts.items():
                entering_count = object_counter.get(vehicle_type, 0)
                leaving_count = object_counter1.get(vehicle_type, 0)
                total_count = count

                writer.writerow({
                    'Vehicle Type': vehicle_type,
                    'Entering Count': entering_count,
                    'Leaving Count': leaving_count,
